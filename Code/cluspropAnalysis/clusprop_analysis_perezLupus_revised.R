here::i_am("Code/cluspropAnalysis/clusprop_analysis_perezLupus_revised.R")
...
if(!file.exists(here::here("results","Revision","cluspropFinal",paste0(dataset_name,".h5ad")))){
    ...
    sce <- readRDS(here::here("data","Processed_Datasets",dataset_name,
        paste0(dataset_name,"_default"), paste0(dataset_name,"_default.Rds")))
    colData(sce) <- clustering_results

    zero_counts <- Matrix::Matrix(c(0), nrow(sce), ncol(sce))
    reduced_sce <- SingleCellExperiment::SingleCellExperiment(
        list(counts=zero_counts),
        colData=colData(sce))

    reducedDim(reduced_sce, "PCA") <- reducedDim(sce, "PCA")
    reducedDim(reduced_sce, "HARMONY") <- reducedDim(sce, "HARMONY")

    sceasy::convertFormat(reduced_sce, from="sce", to="anndata",
