here::i_am("Code/cluspropAnalysis/clusprop_analysis_stephensonCOVIDPBMCfilter_revised.R")

source(here::here("Code","cluspropAnalysis","clusprop_helpers.R"))
dataset_name <- "stephensonCOVIDPBMC"
save_name <- "stephensonCOVIDPBMCfilter"
...
clustering_results <- clustering_results[clustering_results[,"Status"] %in% c("Covid","Healthy"),]
...
unsorted_sample_data <- unique(clustering_results[,c("sample","patient","group","batch","Status")])
...
dist_mat_GMM_PCA_orig <- readRDS(here::here("results","Processed_Datasets",dataset_name,
    paste0(dataset_name,"_default"),paste0(dataset_name,"_default_PCA_GMM_matrix.Rds")))
dist_mat_GMM_PCA_orig <- dist_mat_GMM_PCA_orig[sorted_samples, sorted_samples]
...
dist_mat_KNN_PCA_orig <- readRDS(here::here("results","Processed_Datasets",dataset_name,
    paste0(dataset_name,"_default"),paste0(dataset_name,"_default_PCA_KNN_matrix.Rds")))
dist_mat_KNN_PCA_orig <- dist_mat_KNN_PCA_orig[sorted_samples, sorted_samples]
...
if(!file.exists(here::here("results","Revision","cluspropFinal",paste0(save_name,".h5ad")))){
    library(sceasy)
    library(Seurat)
    ...
    seurat_object <- seurat_object[,seurat_object[[]][,"Status"] %in% c("Covid","Healthy")]
    seurat_object[[]] <- clustering_results
    seurat_object[["RNA"]] <- as(seurat_object[["RNA"]], "Assay")
    sceasy::convertFormat(seurat_object, from="seurat", to="anndata",
        outFile=here::here("results","Revision","cluspropFinal",paste0(save_name,".h5ad")))
}
anndata <- sc$read_h5ad(here::here("results","Revision","cluspropFinal",paste0(save_name,".h5ad")))
...
saveRDS(results_df_all, here::here("results","Revision","cluspropFinal",paste0(save_name,"_results.RDS")))