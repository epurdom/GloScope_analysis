...
build_seurat_object <- function(dataset_name,program_name,assay_name="RNA",save_formats=c("seurat")){
	...
	#meta_sce <- zellkonverter::readH5AD(here::here("data","Processed_Datasets","perezLupus","raw_files","local.h5ad"))
        #metadata_object <- meta_sce@colData
	#rownames(metadata_object) = NULL	
	#saveRDS(metadata_object,here::here("data","Processed_Datasets","perezLupus","raw_files","metadata_raw.Rds"))
	metadata_object <- readRDS(here::here("data","Processed_Datasets","perezLupus","raw_files","metadata.Rds"))
    
	log_object <- HDF5Array::H5ADMatrix(here::here("data","Perez2022","scRNA_raw_cleaned.h5ad"), "data")
	count_object <- HDF5Array::H5ADMatrix(here::here("data","Perez2022","scRNA_raw_cleaned.h5ad"), "counts")
	...
	#sce_object <- zellkonverter::readH5AD(here::here("data","Perez2022","scRNA_raw_cleaned.h5ad"))
        #sce_object <- readRDS(here::here("data","Processed_Datasets","perezLupus","raw_files","init_sce.Rds"))
	set.seed(2023)
	subset_ngenes <- 3000
	subset_genes <- sample(1:dim(sce_object)[1],subset_ngenes,replace=FALSE)
	subset_ncells <- 7000
	subset_cells <- sample(1:dim(sce_object)[2],subset_ncells,replace=FALSE)
	sce_object <- sce_object[subset_genes,subset_cells]
        ...
        metadata_subset <- colData(sce_object)
	rownames(metadata_subset) = NULL
	metadata_subset <- as.data.frame(metadata_subset)
	print(str(metadata_subset))
	saveRDS(metadata_subset,here::here("data","Processed_Datasets",dataset_name,name_str,"metadata_subset.Rds"))
