...
build_seurat_object <- function(dataset_name,program_name,assay_name="RNA",save_formats=c("seurat")){
	...
	# IMPORANT: Use `here::here()` for file paths relative to `pop_data`
        ...
        count_data <- Seurat::Read10X(data.dir = here::here("data","Processed_Datasets","wuBreastCancer","raw_files"))
        ...
        metadata_df <- read.csv(here::here("data","Processed_Datasets","wuBreastCancer","raw_files","whole_miniatlas_meta.csv"),row.names=1)
...
metadata_manipulation_fn <- function(metadata_df){
	metadata_df$sample <- metadata_df$Patient
	return(metadata_df)
}
...
get_seurat_pipeline_params <- function(){
	seurat_params <- list(
		NormalizeData = list(verbose=FALSE),
		FindVariableFeatures = list(verbose=FALSE),
		ScaleData = list(verbose=FALSE),
		RunPCA = list(verbose=FALSE),
		RunUMAP = list(reduction = "pca",dims=1:10,verbose=FALSE),
		FindNeighbors = list(verbose=FALSE),
		FindClusters = list(verbose=FALSE)
	)
