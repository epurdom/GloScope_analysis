...
get_seurat_pipeline_params <- function(){
	seurat_params <- list(
		NormalizeData = list(normalization.method="RC",scale.factor=10000),
		FindVariableFeatures = list(selection.method = "vst",nfeatures=3000),
		ScaleData = list(),
		RunPCA = list(),
		RunUMAP = list(reduction = "pca",dims=1:10),
		FindNeighbors = list(),
		FindClusters = list()
	)
...
get_seurat_pipeline_custom_fns <- function(){ 
	custom_fns_list <- list(
		Pre = list(fn_name=seurat_preprocessing,params=list(min_cells_cutoff=10)), # We filter out genes appearing in less than 10 cells
		NormalizeData = NULL,
		ScaleData = NULL,
		RunPCA = NULL,
		RunUMAP = NULL,
...
seurat_preprocessing <- function(seurat_object,min_cells_cutoff){            
        # remove genes that appear in less than `min_cells_cutoff` cells  
        expression_mask <- Matrix::rowSums(seurat_object@assays$RNA@counts > 0)
        seurat_object <- seurat_object[expression_mask >= min_cells_cutoff,]
                                                                             
        # remove samples with less than 1000 cells                           
        low_cell_samples <- c("C113_N","C115_N","C119N","C133_N","C135N","C151_T")
        low_cell_mask <- !(seurat_object[[]]$sample %in% low_cell_samples)
        ...
        sample_mask <- (seurat_object[[]]$patient != "C162")                 
