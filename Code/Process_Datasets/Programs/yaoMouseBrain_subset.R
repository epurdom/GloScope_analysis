...
build_seurat_object <- function(dataset_name,program_name,assay_name="RNA",save_formats=c("seurat")){
	...
	metadata_df = colData(sce_object)
	rownames(metadata_df) = NULL                                                     
	metadata_df <- as.data.frame(metadata_df)
	saveRDS(metadata_df, here::here("data","Processed_Datasets","yaoMouseBrain","raw_files","metadata.Rds"))                     
...
metadata_manipulation_fn <- function(metadata_df){
	metadata_df$sample <- interaction(metadata_df$donor_label,metadata_df$region_label,sep="_")
	return(metadata_df)
}
...
sce_pca <- function(sce_object){
	...
	# harmony expects labels as a characrter vector, not factor
	#sce_meta <- colData(sce_object)
	#sce_meta$donor_label <- as.character(sce_meta$donor_label)
	#colData(sce_object) <- sce_meta
	sce_object <- harmony::RunHarmony(sce_object,group.by.vars="patient")
	return(sce_object)
}
