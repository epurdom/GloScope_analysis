...
build_seurat_object <- function(dataset_name,program_name,assay_name="RNA",save_formats=c("seurat")){
	...
	saveRDS(metadata_df, here::here("data","Processed_Datasets","perezLupus","perezLupus_subset","metadata_subset.Rds"))
...
metadata_manipulation_fn <- function(metadata_df){
	metadata_df$final_sample <- interaction(metadata_df$sample_uuid, metadata_df$Processing_Cohort)
	metadata_df$final_sample <- droplevels(metadata_df$final_sample)
	return(metadata_df)
}
