...
build_seurat_object <- function(dataset_name,program_name,assay_name="RNA",save_formats=c("seurat")){
	...
	# IMPORANT: Use `here::here()` for file paths relative to `pop_data`
        ...
        count_data <- readRDS(here::here("data","Processed_Datasets","liuRash31","raw_files","counts.Rds")) 
        ...
        metadata_df <- readRDS(here::here("data","Processed_Datasets","liuRash31","raw_files","metadata.Rds"))
...
metadata_manipulation_fn <- function(metadata_df){
	...
	factor_cols <- c("treat","chem","order","spike")
	metadata_df[factor_cols] <- lapply(metadata_df[factor_cols], factor)

	metadata_df["sample"] <- metadata_df["donor"]
	return(metadata_df)
}
...
run_harmony <- function(seurat_object){ 
        seurat_object <- harmony::RunHarmony(seurat_object,"patient")
        seurat_object <- Seurat::ProjectDim(seurat_object,reduction="harmony",overwrite=T,verbose=F)
        return(seurat_object)
}
