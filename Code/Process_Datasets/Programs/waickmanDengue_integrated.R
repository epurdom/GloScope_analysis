...
run_sct <- function(seurat_object){
	# run SCT transform to individual data sets and integration as outlined in the Supplementary Materials
	# Code modified from `https://satijalab.org/seurat/articles/integration_introduction.html`
	sample_list <- Seurat::SplitObject(seurat_object)
	sample_list <- lapply(X = sample_list, FUN = Seurat::SCTransform)
	reference_samples <- NA
	integration_features <- Seurat::SelectIntegrationFeatures(object.list = samples_list)
	sample_list <- Seurat::PrepSCTIntegration(object.list = sample_list, anchor.features = integration_features)
	anchors <- Seurat::FindIntegrationAnchors(object.list = sample_list, normalization.method = "SCT",
