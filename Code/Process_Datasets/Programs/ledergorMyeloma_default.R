...
build_seurat_object <- function(dataset_name,program_name,assay_name="RNA",save_formats=c("seurat")){
	...
	name_str <- paste0(dataset_name,"_",program_name)
        ...
        txt_files <- list.files(path=here::here('data','Processed_Datasets','ledergorMyeloma','raw_files','GSE117156_RAW'),pattern="*.txt",full.names=T)
	count_dfs <-  lapply(txt_files,read.table,header=T,row.names=1) # list of cell x gene data.frames for each unit
	count_data <- do.call(cbind, count_dfs) # combine into a single data.frame
        ...
        metadata_df <- read.table(here::here('data','Processed_Datasets','ledergorMyeloma','raw_files','GSE117156_meta','GSE117156_metadata.txt'),header=T,row.names=1) 
...
metadata_manipulation_fn <- function(metadata_df){
	# to get the indicator of phenotype, we note that healthy samples all come from the hip
        # we parse the `Experiment_ID` variable accordingly
        id_list <- metadata_df[,"Experiment_ID"]
        patient_id <- unlist(lapply(id_list,function(x) { strsplit(x,"_")[[1]][2] } ))
	metadata_df[,"patient_id"] <- patient_id
        group_id <- unlist(lapply(patient_id, function(x){ substring(x, 1, nchar(x) - 2) } )) # last two digits are numeric id
        metadata_df[,"condition"] <- group_id   
        metadata_df[,"condition"] <- as.factor(metadata_df[,"condition"])
        metadata_df[,"healthy"] <- unlist(lapply(metadata_df[,"condition"], function(x) { x == "hip" } ))
        metadata_df[,"healthy"] <- as.factor(metadata_df[,"healthy"])
	# if postRx is in the Experiment_ID, then this is a post-therapy observation
	metadata_df[,"timepoint"] <- as.factor(ifelse(sapply(metadata_df$Experiment_ID,function(x){grepl("postRx",x,fixed=TRUE)}),"post","pre"))
	metadata_df[,"sample_type"] <- interaction(metadata_df[,"condition"], metadata_df[,"timepoint"], sep=".")
	# two tissue types are considered: circulating plasma cells (circPC) and bone marrow plasma cells (BMPC)
	metadata_df[,"tumor_type"] <- as.factor(ifelse(sapply(metadata_df$Experiment_ID,function(x){grepl("circPC",x,fixed=TRUE)}),"circPC","BMPC"))
...
get_seurat_pipeline_custom_fns <- function(){ 
	custom_fns_list <- list(
		Pre = list(fn_name=seurat_preprocessing,params=list()),
		NormalizeData = NULL,
		ScaleData = NULL,
		RunPCA = NULL,
		RunUMAP = NULL,
...
seurat_preprocessing <- function(seurat_object){ 
	# remove 2 samples with less than 50 cells
	seurat_object <- subset(seurat_object, subset = (sample != "AB3195"))
	seurat_object <- subset(seurat_object, subset = (sample != "AB3178"))
	seurat_object <- subset(seurat_object, subset = (sample != "AB3461")) # unit causing extreme distances

	seurat_object <- subset(seurat_object, subset = num_reads >= 500) # filter used by original authors
