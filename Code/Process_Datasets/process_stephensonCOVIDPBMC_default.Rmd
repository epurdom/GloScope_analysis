---
title: "Process COVID-infected PBMC Data"
author: "Purdom Group"
date: "`r Sys.Date()`"
...
options(error = quote(dump.frames("covid143ErrorDump", TRUE))) # error logging
library(here)
...
formats_to_save = list("h5Seurat") # All options 
...
patient_str <- "patient_id" # The string of the patient ID covariate
group_str <- "Status_on_day_collection_summary" # The group (phenotype/condition) covariate
sample_str <- "sample_id" # The sample ID within a patient
cell_type_str <- "initial_clustering" # The cell type covariate
batch_str <- "Site" # The covariate associated with any possible batch effect such as processing site or date
num_features_str <- "n_genes" # The covariate for the number of non-zero features in a given cell. `nFeature_RNA` is the Seurat default
num_reads_str <- "total_counts" # The covariate for the number of reads in a given cell. `nCount_RNA` is the Seurat default
...
saveRDS(seurat_object, file=here::here("data","Processed_Datasets",dataset_name,name_str,paste0(name_str,".Rds")) )
```
