---
title: "Process Perez et al. (2022) Lupus PBMC Data"
author: "Purdom Group"
date: "`r Sys.Date()`"
...
options(error = quote(dump.frames("pelkaErrorDump", TRUE))) # error logging
library(here)
...
# Bellow the Boolean `check_initial_sce_save` determines whether to check for a "baseline" Seurat object containing the unmodified reads and metadata

dataset_name <- "perezLupus" 
program_name <- "default" 
primary_assay <- "RNA" 
check_initial_sce_save <- FALSE
...
formats_to_save = list("sce") # All options 

sce_object <- NULL
initial_sce_save_path <- here::here("data","Processed_Datasets",dataset_name,name_str,"initial_sce")

initial_sce_save_found <- FALSE # For printing in text
rds_save_used <- TRUE # For printing in text
# Rds appears to give faster load times than h5Seurat, so we check that format first.
if (check_initial_sce_save && file.exists(paste0(initial_sce_save_path,".Rds"))) {
	sce_object <- readRDS(paste0(initial_sce_save_path,".Rds"))
	initial_sce_save_found <- TRUE
} 

if (is.null(sce_object)){ # Will still be NULL if saved data has not been loaded
	sce_object <- build_seurat_object(dataset_name,program_name,primary_assay,save_formats=formats_to_save)
}
...
`r if(!initial_sce_save_found){"An unmodified SCE object was computed de novo in this run."}`
`r if(!initial_sce_save_found){paste0("It was saved in the following formats at Code/Process_Datasets/",dataset_name,"/",name_str,": ",toString(formats_to_save),".")}`

`r if(initial_sce_save_found && rds_save_used){paste0("A previously computed and unmodified SCE object was loaded from Code/Process_Datasets/",dataset_name,"/",name_str,"/initial_sce.Rds.")}`
...
patient_str <- "donor_uuid" # The string of the patient ID covariate
group_str <- "disease_state" # The group (phenotype/condition) covariate
sample_str <- "final_sample" # The sample ID within a patient
cell_type_str <- "cell_type" # The cell type covariate
batch_str <- "Processing_Cohort" # The covariate associated with any possible batch effect such as processing site or date
...
metadata_df <- readRDS(here::here("data","Processed_Datasets","perezLupus","raw_files","metadata_df.Rds"))
metadata_formatted_df <- format_metadata_df(metadata_df,
...
metadata_formatted <- S4Vectors::DataFrame(metadata_formatted_df) # convert from data.frame to DFrame
sce_object@colData <- metadata_formatted # update object
```


```{r save_metadata,echo=FALSE}
# save the meta.data standalone for access without loading a Seurat object
saveRDS(metadata_formatted_df,file=here::here('data','Processed_Datasets',dataset_name,'metadata',paste0(name_str,"_metadata.Rds")))
```

```{r check_sample_bool,echo=FALSE}
single_sample_bool <- get_patient_sample_cross(metadata_formatted_df)
```
...
Before running the pre-processing pipeline there are `r dim(sce_object)[2]` cells and `r dim(sce_object)[1]` features in the dataset.
...
sce_object <- run_seurat_pipeline(sce_object,seurat_params,custom_fns,is_sce=TRUE)
pipeline_end <- proc.time()[3]
pipeline_delta <- pipeline_end - pipeline_start
saveRDS(sce_object, file=here::here("data","Processed_Datasets",dataset_name,name_str,paste0(name_str,".Rds")) )
```

After running the entire pipeline there are `r dim(sce_object)[2]` cells and `r dim(sce_object)[1]` features. It took `r pipeline_delta` seconds to run. 
...
aggregation_variable <- "sample"
subset_variable <- "cell_type"

if (!is.null(aggregation_variable) && !is.null(subset_variable)){
	proportion_df <- get_cluster_props(metadata_formatted_df, aggregation_variable,subset_variable) # The first covariate is the one to aggregate along, the second is the subsets (typically patients and cell types)
