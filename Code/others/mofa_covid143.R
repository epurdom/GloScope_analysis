library(dplyr)
library(stringr)
myseurat <- readRDS("../../data/Processed_Datasets/stephensonCOVIDPBMC/metadata/stephensonCOVIDPBMC_default_metadata.Rds")
load("../../results/BatchStudy/COVID143_pseudobulk_manual.Rda")
meta <- unique(myseurat[, c("Status", "sample")])
for(i in names(pb_cluster)){
  colnames(pb_cluster[[i]]) = paste0(colnames(pb_cluster[[i]]), ".", i)
}
pb_count <- do.call(cbind, pb_cluster)
metadata <- data.frame(id = colnames(pb_count), cell_counts = colSums(pb_count))
metadata$donor_id <- unlist(lapply(str_split(metadata$id, "[.]"), function(x) x[1]))
metadata$cell_type <- unlist(lapply(str_split(metadata$id, "[.]"), function(x) x[2]))
...
save(model, file = "../../results/BatchStudy/mofa_test_covid143.Rda")
