...
A_clus = c(50,10,5,10,15,10,5,10)
...
sim  = sim %>%
  ...
  ScaleData(verbose = T)%>%
  RunPCA(verbose = T) %>%
  FindNeighbors(verbose = T) %>%
  FindClusters(verbose = T) 

pca_clus = sim$seurat_cluster
...
clus_df = data.frame("pca" = pca_clus)

plot_df = cbind(sim@meta.data, sim@reductions$pca@cell.embeddings, latent_batch)

clus_df = cbind(plot_df, clus_df)
save(clus_df, file = paste0("../../results/simulation/tmp/sil/",slurm_arrayid,".Rda"))
set.seed(1)
...
save(clus_true,clus_seurat_pca, clus_seurat_scvi, 
     file =  paste0("../../results/simulation/distmat/clusprop_V3/Setting1/25_50/cluster/",slurm_arrayid ,".Rda" ))

save(dist_mat_KNN_pca,dist_mat_GMM_pca,
  dist_mat_KNN_scvi,dist_mat_GMM_scvi, 
    file = paste0("../../results/simulation/distmat/clusprop_V3/Setting1/25_50/Glos/",slurm_arrayid,".Rda"))
