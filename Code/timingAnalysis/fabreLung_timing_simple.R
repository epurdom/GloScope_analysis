...
count_data <-  ReadMtx(
          mtx = here::here("data","Processed_Datasets","fabreLung","raw_files","expression","64131be8373a3aaf4ed5a9c6","matrix__2_.mtx.gz"), 
          features = here::here("data","Processed_Datasets","fabreLung","raw_files","expression","64131be8373a3aaf4ed5a9c6","features.tsv.gz"),
          cells = here::here("data","Processed_Datasets","fabreLung","raw_files","expression","64131be8373a3aaf4ed5a9c6","barcodes.tsv.gz"))
        norm_data <-  ReadMtx(
          mtx = here::here("data","Processed_Datasets","fabreLung","raw_files","expression","6414720b9be282e93b39829f","matrix.mtx.gz"), 
          features = here::here("data","Processed_Datasets","fabreLung","raw_files","expression","6414720b9be282e93b39829f","features_lung_processed.tsv.gz"),
          cells = here::here("data","Processed_Datasets","fabreLung","raw_files","expression","6414720b9be282e93b39829f","barcodes_lung_processed.tsv.gz"))
        count_data <- count_data[rownames(norm_data),]

 metadata_df <- read.table(here::here("data","Processed_Datasets","fabreLung","raw_files","metadata","2022_SI_human_lung_allcells_metadata_file_updated.txt"),header = T, sep="\t", quote = "")[-1,]
metadata_df["sample"] <- metadata_df["biosample_id"]

seurat_object <- Seurat::CreateSeuratObject(counts=count_data,data = norm_data,                                                 project="timing_test",assay="RNA",meta.data=metadata_df)
